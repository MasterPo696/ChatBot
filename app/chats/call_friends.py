...
async def friend_selected(call: CallbackQuery):
    ...
    name = friend[1]
    if success:
        await call.message.answer(f"<b>You've sent the date request to {name}.</b>", reply_markup=cancel_markup, parse_mode="HTML")
    else:
        await call.message.answer("<b>Failed to send the request. The friend might not be available.</b>", parse_mode="HTML")
...
@call_router.message(Reply.message)
async def reply_after_dismiss(state: FSMContext):
    data = await state.get_data()
    user_id =data.get("user_id")
    print(user_id)

# Step 6: Handle Dismiss Request
@call_router.callback_query(F.data == ("reply"))
async def reply_after_dismiss(call: CallbackQuery, state: FSMContext):


    # Notify the requester about the dismissal
    # await bot.send_message(requester_id, "<b>Your chat request has been dismissed by the recipient.</b>", parse_mode="HTML")
    await call.message.answer("<b>Write down your reply.</b>", parse_mode="HTML")
    await state.get_state(Reply.message)
    await call.answer()


# Step 6: Handle Dismiss Request
@call_router.callback_query(F.data.startswith("dismiss_request:"))
async def dismiss_request(call: CallbackQuery, state: FSMContext):
    _, requester_id, friend_id = call.data.split(":")
    requester_id = int(requester_id)

    await state.update_data(user_id=requester_id, partner_id=friend_id)
    # Notify the requester about the dismissal
    await bot.send_message(requester_id, "<b>Your chat request has been dismissed by the recipient.</b>", parse_mode="HTML")
    await call.message.answer("<b>You have dismissed the request.</b>", reply_markup=kb.reply, parse_mode="HTML")
    await call.answer()
...
async def cancel_request(call: CallbackQuery):
    ...
    await bot.send_message(friend_id, "<b>The chat request has been canceled by the requester.</b>", parse_mode="HTML")
    
    # Notify the requester about the cancellation
    await bot.send_message(requester_id, "<b>You have canceled the chat request.</b>", parse_mode="HTML")
...
async def back_to_menu(call: CallbackQuery):
    ...
    await call.message.answer("<b>Returning to the main menu.</b>", parse_mode="HTML", reply_markup=kb.menu)
    await call.answer()
