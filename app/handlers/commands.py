...
async def handle_invite_button(callback: CallbackQuery, state: FSMContext):
    ...
    result = db.create_profile(user_id, user_name, sex, referral_id_data)
    await state.clear()
...
async def handle_invite_button(callback: CallbackQuery, state: FSMContext):
    ...
    # result1= db.update_interested_in(user_id, result)
    # print(result1)
    # result2 = db.get_looking_for(user_id)
    # print(result2)
    await db.update_interested_in(user_id, result)
    ...
    if chat == False:
        if exist:
            if interested_in[0]:
                ...
                if looking_for == None:
                    await message.answer("<b>Choose the partner gender!</b> ", reply_markup=kb.chat_menu, parse_mode="HTML")
                else:
